...
# 플러그인/병렬 처리 지원
#
# 2025-10-06: 기존 엔진은 함수가 큰 덩어리로 묶여 있어 유지보수와
# 성능 개선이 어려웠다. 주요 파일 패치 계획서에서 제안한 바와 같이
# 각 단계(수집, 정규화, 품질 검사(QG), 팩트체크(FC), 키워드/도메인
# 스코어링 등)를 플러그인 형태로 정의하고, 여러 기사를 병렬로
# 처리할 수 있도록 리팩터링한다. `PluginManager`는 기본 함수들을
# 등록하고, config.json의 `plugins` 섹션을 통해 외부 모듈의 함수로
# 대체할 수 있다. 또한 `concurrent.futures`를 이용하여 기사
# 처리단계를 병렬화해 수집 속도를 높인다. 앞으로 추가될 요약/번역
# 모듈 역시 플러그인으로 확장할 수 있다.
import importlib
from concurrent.futures import ThreadPoolExecutor, as_completed

class PluginManager:
    """
    플러그인을 통한 단계별 처리 함수 관리.

    config의 `plugins` 키에 {"stage_name": "module:function"} 형식으로
    정의하면 기본 함수 대신 해당 모듈의 함수를 사용한다. 예를 들어
    config.json에 다음과 같이 지정하면 커스텀 품질 게이트를 적용할 수
    있다:

    "plugins": {
        "quality_gate": "my_plugins.custom_quality_gate",
        "fact_check": "my_plugins.custom_fact_check"
    }
    """

    DEFAULTS = {
        "quality_gate": None,  # placeholder; will be set after definitions
        "fact_check": None,
        "keyword_score": None,
        "domain_score": None,
    }
    ...
    def __init__(self, cfg: dict | None = None) -> None:
        # 기본 함수는 아래에서 등록한다. 이후 config의 override를 반영한다.
        self.funcs: dict[str, callable] = dict(self.DEFAULTS)
        # 나중에 quality_gate 등 실제 함수들을 할당하기 위해 후행 코드에서
        # PluginManager.DEFAULTS를 교체한다.
        # config에서 플러그인 지정
        plugs = (((cfg or {}).get("plugins")) or {}) if cfg else {}
        for name, path in plugs.items():
            try:
    ...
    def register_default(self, name: str, func: callable) -> None:
        """기본 함수를 등록한다. 나중에 config에서 override될 수 있다."""
        if name not in self.funcs or self.funcs[name] is None:
            self.funcs[name] = func

    def run(self, name: str, *args, **kwargs):
        func = self.funcs.get(name)
        if not func:
            raise ValueError(f"플러그인 '{name}'를 찾을 수 없습니다.")
...
DEFAULT_CFG = {
    ...
    "features": {
        "section_order": ["표준 뉴스", "글로벌 전자생산", "AI 뉴스", "커뮤니티 워치", "퀄리뉴스"],
        "quality": {
            "min_chars": 600,
            "min_sentences": 5,
            "min_links": 2,
        ...
        "factcheck": {"enabled": True, "min_score": 52, "min_evidence": 2, "strict": True},
        "selection_file": "selected_articles.json",
        "translate_model": os.getenv("OPENAI_TRANSLATE_MODEL", "gpt-4o"),
        "landing_rules": {
            "ipcglobalinsight.org": {
                # 연/월/기사 경로만 허용, Newsletter 인덱스는 배제
                "prefer_patterns": [
                    "^/newsletters/[^/]+/20\\d{2}/\\d{2}/[a-z0-9-]+/?$",
                    "^/20\\d{2}/\\d{2}/[a-z0-9-]+/?$",
                    "^/news/20\\d{2}/\\d{2}/[a-z0-9-]+/?$",
                ],
                "deny_patterns": ["^/newsletters/global-insight/?$", "^/newsletters/?$", "^/$"],
            },
            "ipc.org": {
                "prefer_patterns": ["/ipc-insights/[a-z0-9-]+/?$", "/news/[a-z0-9-]+/?$"],
                "deny_patterns": ["/ipc-insights/?$", "/news/?$"],
            },
            "ecss.nl": {
                "prefer_patterns": ["/standard/[a-z0-9-]+/?$", "/hb/[a-z0-9-]+/?$"],
                "deny_patterns": ["/standards/active-standards/?$"],
            },
            "iconnect007.com": {
                "prefer_patterns": ["^/article/\\d+(/[a-z0-9-]+)?/?$", "^/article/[a-z0-9-]+/\\d+/?$"],
                "deny_patterns": ["^/page/(pcb007|smt007)/\\d+/?$", "/gallery/", "/video/", "/webinar/", "/jobs/"],
            },
            "openai.com": {"prefer_patterns": ["^/blog/.+", "^/research/.+"], "deny_patterns": ["/blog/?$", "/research/?$"]},
            "ai.meta.com": {"prefer_patterns": ["^/blog/.+"], "deny_patterns": ["/blog/?$"]},
            "blog.google": {"prefer_patterns": ["^/products/news/.+"], "deny_patterns": ["/products/news/?$"]},
        },
    },
    # 커뮤니티 분리 운영 기본값
    "community": {
        "enabled": True,
        "selection_file": "selected_community.json",
        "html_title": "퀄리 커뮤니티",
        "min_upvotes": 15,
        "min_comments": 4,
        "fresh_hours": 168,  # 7일
...
# 기본 플러그인 함수 등록.
#
# PluginManager는 인스턴스 생성 시 DEFAULTS를 복사한 뒤 config의
# 오버라이드 값으로 대체한다. 여기서 DEFAULTS에 실제 함수를
# 할당해두면 register_default를 호출하지 않아도 기본 함수를 사용할 수
# 있다. fetch_articles에서는 여전히 register_default를 통해 명시적으로
# 설정하지만, 다른 곳에서 PluginManager를 사용할 때를 위해 기본값을
# 세팅한다.
try:
    PluginManager.DEFAULTS.update({
    ...
    # PluginManager가 아직 정의되지 않은 초기 import 단계일 수 있으므로
    pass
...
def fetch_articles(cfg: dict | None = None) -> List[Dict]:
    ...
    기존 구현은 각 공식 소스를 순차적으로 처리했지만, 2025년 리팩터링에
    따라 다음과 같은 개선을 포함한다:

    1) `PluginManager`를 사용해 품질 게이트, 팩트체크, 키워드/도메인
       스코어링 함수를 유연하게 오버라이드할 수 있다.
    2) ThreadPoolExecutor로 기사 처리 병렬화를 지원하여 I/O 대기
       시간을 줄인다. 환경변수 `QUALI_MAX_WORKERS`를 통해 워커 수를
       제어할 수 있다.
    3) 기사별 처리를 `_process_article` 내부 함수로 분리하여 가독성과
       테스트 용이성을 높였다.
    """
    feeds_dir = os.path.join(os.getcwd(), "feeds")
    RULES = _load_editor_rules()
    ...
    # 플러그인 매니저 초기화 및 기본 함수 등록
    pm = PluginManager(cfg)
    # 기본 함수 등록 (config에서 오버라이드하지 않은 경우 사용됨)
    pm.register_default("quality_gate", quality_gate)
    pm.register_default("fact_check", fact_check)
    pm.register_default("keyword_score", _keyword_score)
    pm.register_default("domain_score", _domain_score)
    ...
    # 후보 기사 URL 수집
    candidate_tasks: List[tuple[str, str]] = []
    for label, url in sources.items():
        ...
        try:
            ...
            for art_url in cand_urls:
                candidate_tasks.append((label, canonical_url(art_url)))
        except Exception as e:
            logger.warning("fetch_articles listing fail: %s | %s", url, e)
            continue

    def _process_article(label: str, art_url: str) -> Dict | None:
        """개별 기사 처리 함수. 실패 시 None 반환."""
        try:
            ...
            # 플러그인 기반 품질 게이트
            qg = pm.run("quality_gate", art_url, raw_html, title, label, cfg)
            # 요약문 추출
            ...
            # 플러그인 기반 팩트체크
            fc = pm.run("fact_check", art_url, raw_html, title, three_ko, cfg)
            ...
            # 플러그인 기반 키워드/도메인 점수
            kw_s, kw_hits = pm.run("keyword_score", title, eng_src, RULES)
            dom_s = pm.run("domain_score", art_url, RULES)
            ...
            logger.warning("process_article fail: %s | %s", art_url, ex)
            return None

    # 병렬 처리
    max_workers = int(os.getenv("QUALI_MAX_WORKERS", "4"))
    # 워커 수는 최소 1, 최대 후보 기사 수
    max_workers = max(1, min(max_workers, len(candidate_tasks)))
    if not candidate_tasks:
        return []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_task = {executor.submit(_process_article, label, art_url): (label, art_url) for (label, art_url) in candidate_tasks}
