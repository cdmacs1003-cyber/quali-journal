...
7. For governance-pack edits: `QUALI_UI_GOVERNANCE_ADVANCEMENT_STRATEGY.md`

If these files conflict, apply this precedence:

```text
Constitution > Guardrails > AGENTS > Guidebook > Workflow > Codex Final > Memory > Skill > Strategy
```

Memory records past decisions. It does not override the Constitution or Guardrails.
Strategy records roadmap direction. It does not override active governance documents.
...
- If governance documents change, what schema, hash, or document-sync check must also change?
...
- Keep status values aligned across Constitution, Guardrails, Guidebook, Memory, and Skill.
- Update `SHA256SUMS.txt` after verified governance-file changes.
...
- Governance-pack changes keep status, token, component, and Evidence Schema definitions synchronized.
- Governance-pack changes update `SHA256SUMS.txt` after verification.

---

## 9. Governance Automation

When available, run these checks before reporting completion:
...
If a check cannot run, report it as HOLD with the reason.
...
- Added governance-pack reading rule for the advancement strategy
- Added Skill and Strategy to conflict precedence
- Added document sync and hash integrity requirements
- Added governance automation check commands

### v0.1.0 — 2026-06-29

- Initial AGENTS execution guide
