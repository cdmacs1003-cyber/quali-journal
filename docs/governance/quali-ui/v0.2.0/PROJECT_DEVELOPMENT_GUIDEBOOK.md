...
### 4.3 한글 타이포그래피와 행갈이 토큰

퀄리 UI는 한글 제목과 본문을 브라우저 기본 줄바꿈에 맡기지 않는다.  
대형 제목은 의미 단위로 행을 나누고, 핵심 용어가 중간에서 끊기지 않도록 관리한다.
...
| H1 | 의미 단위 2~3줄 |
| H2 | 의미 단위 1~2줄 |
| H3 | 가능한 한 1줄, 필요 시 의미 단위 2줄 |
| 본문 | 한글 기준 과도하게 길거나 짧지 않게 조정 |
| 모바일 | 제목이 4줄 이상 무너지지 않도록 별도 확인 |
| 코드·경로·표준번호 | `overflow-wrap:anywhere` 허용 |
| 핵심 용어 | `.nowrap` 또는 `.term`으로 보호 가능 |
...
### 5.3 Standard Code Badge

목적: 표준번호, 개정, 적용 Class 같은 짧은 기준 식별자를 일관된 방식으로 표시한다.
...
```text
IPC-A-610 · Rev. H · Class 3
```

### 5.4 Workbench Search
...
| REVIEWED | 검토 완료 | READY 또는 PASS가 아님 |
...
| 필드 | 목적 |
|---|---|
| `standard_id` | 표준번호 또는 기준 식별 |
| `revision` | 개정·버전 |
| `clause` | 조항·절·근거 위치 |
| `source_file` | 원본 파일 또는 출처 |
| `page` | 페이지·범위·위치 |
| `evidence_level` | direct, summarized, inferred, pending |
| `verification_status` | PASS, HOLD, FAIL |
| `limitation` | 근거 한계·주의사항 |
| `next_action` | 다음 확인 또는 조치 |
...
### 5.12 Line Break Gate

목적: 한글 제목과 핵심 문장의 줄바꿈 품질을 최종 검사한다.

검사 항목:
- H1이 의미 단위 2~3줄로 구성되었는가?
- H2가 의미 단위 1~2줄로 구성되었는가?
- 조사·접미사·핵심 명사가 단독 줄로 떨어지지 않는가?
- 보호 용어가 중간에서 끊기지 않는가?
- 모바일에서 제목이 4줄 이상 무너지지 않는가?
- 본문 줄 길이가 과도하게 길거나 짧지 않은가?
- 최종 인상이 미완성 시안처럼 보이지 않는가?
...
- 대형 제목은 의미 단위로 직접 줄바꿈한다.
- `문장으 / 로`, `리 / 스크`, `시각요 / 약`처럼 글자 단위로 끊긴 제목은 FAIL이다.
- 핵심 용어는 가능한 한 한 줄에 유지한다.
- 본문에는 `word-break: keep-all`을 적용한다.
- 코드, 경로, 표준번호는 예외적으로 `overflow-wrap:anywhere`를 허용한다.
- 최종 출력 전 Line Break Gate를 수행한다.
...
| 한글 행갈이 | 제목과 핵심 문장이 의미 단위로 줄바꿈되는가? | PASS/HOLD/FAIL |
| 근거 스키마 | Evidence 필수 필드가 충족되는가? | PASS/HOLD/FAIL |
| 문서 정합성 | 상태·토큰·컴포넌트 기준이 동기화되었는가? | PASS/HOLD/FAIL |
| 해시 | `SHA256SUMS.txt`가 최신인가? | PASS/HOLD/FAIL |
...
- `REVIEWED`는 검토 완료이며 READY 또는 PASS를 대체하지 않음
- 토큰 추가는 최소화
- 예외 허용은 문서화 없이 금지
- 공식 제품명 변경 금지
- H1/H2는 브라우저 자동 줄바꿈에 맡기지 않고 의미 단위로 검토
- Line Break Gate 실패 시 READY 또는 PASS 표시 금지
- 문서 변경 후 검증을 실행하고 `SHA256SUMS.txt`를 갱신
- Evidence Schema 변경 시 Guardrails, Memory, Skill, schema 파일을 함께 갱신

---

## 12. 스키마·검증 산출물

Quali UI 기준은 문서와 실행 파일을 함께 사용한다.

| 경로 | 역할 |
|---|---|
| `schemas/tokens.schema.json` | 승인 토큰의 기계 판독 기준 |
| `schemas/status.schema.json` | 공식 UI 상태 값 기준 |
| `schemas/evidence.schema.json` | 근거 데이터 필수 필드 기준 |
| `schemas/component-contract.schema.json` | 공통 컴포넌트 계약 기준 |
| `scripts/check-hash.mjs` | `SHA256SUMS.txt` 무결성 검사 |
| `scripts/check-token-usage.mjs` | 승인되지 않은 HEX 색상 검사 |
| `scripts/check-contrast.mjs` | 승인 색상 대비 검사 |
| `scripts/check-doc-sync.mjs` | 상태·토큰·컴포넌트 문서 정합성 검사 |
| `scripts/check-korean-linebreak.mjs` | 한글 행갈이 정적 검사 |
| `scripts/check-evidence-schema.mjs` | Evidence Schema 필수 필드 검사 |
| `templates/preflight.md` | 작업 전 확인 양식 |
| `templates/verification-report.md` | 작업 후 검증 보고 양식 |
| `templates/memory-entry.md` | Memory 기록 양식 |
| `templates/output-gate.md` | 출력 전 검사 양식 |
...
- `REVIEWED` 상태 의미를 Constitution과 정합화
- Evidence Schema 필수 필드 추가
- 리뷰 체크리스트에 근거 스키마, 문서 정합성, 해시 항목 추가
- 스키마·검증 산출물 목록 추가
- 한글 제목 자간 정책을 `letter-spacing: 0`으로 정리

### v0.1.1 — 2026-06-29

- 한글 타이포그래피와 행갈이 원칙 추가
- `word-break: keep-all`, `span.line`, `.nowrap`, `.term` 적용 기준 추가
- Line Break Gate 컴포넌트 추가
- 리뷰 체크리스트에 한글 행갈이 항목 추가
- 운영 규칙에 제목 행갈이 검토 의무 추가
