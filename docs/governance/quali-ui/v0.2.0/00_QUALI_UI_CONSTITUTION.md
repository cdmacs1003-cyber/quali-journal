...
| HOLD | 확인 필요, 보류 |
| FAIL | 기준 미달, 차단 |
| REVIEWED | 검토 완료, 아직 제출 가능 상태는 아님 |
| READY | 제출 또는 사용 가능 |
| PASS | 기준 충족, 완료 가능 |
| ARCHIVED | 보관, 비활성 |
...
- 문서 간 상태·토큰·컴포넌트 기준이 서로 충돌한다.
- 문서 변경 후 무결성 해시 또는 검증 결과를 갱신하지 않는다.
- 근거 데이터가 Evidence Schema의 필수 필드를 충족하지 않는다.

---

## 11. 거버넌스 고도화 원칙

퀄리 UI의 기준은 사람이 읽는 문서와 기계가 검증하는 파일로 함께 관리한다.

| 기준 | 역할 |
|---|---|
| Constitution | 최상위 정체성, 가치, 위반 판정 |
| Guardrails | 금지·필수 조건, 출력 전 검사 |
| Guidebook | 설계·컴포넌트·운영 가이드 |
| Memory | 반복 결정, 예외, HOLD, 판례 기록 |
| Skill | 에이전트 실행 압축본 |
| Schemas | 상태, 토큰, 근거, 컴포넌트 계약의 기계 판독 기준 |
| Scripts | 해시, 토큰, 대비, 문서 정합성, 근거 스키마 검사 |
| Templates | Preflight, Verification, Memory, Output Gate 반복 양식 |

문서와 스키마가 충돌하면 헌법과 가드레일을 기준으로 스키마와 하위 문서를 갱신한다.  
스키마와 검증 스크립트는 문서의 보조물이 아니라, 문서 기준을 실행 가능한 형태로 내린 운영 장치다.
...
- `REVIEWED`를 공식 허용 상태로 승격
- 상태 의미 표 추가
- 문서 정합성, 해시 갱신, Evidence Schema 위반을 헌법 위반 조건에 추가
- 스키마·스크립트·템플릿을 거버넌스 운영 장치로 명시

### v0.1.0 — 2026-06-29

- 초기 Constitution 생성
- 공식 제품명 고정
- 핵심 정체성, 10대 가치, 절대 흐름, 이중 설명 원칙 정의
